...
class SeedTests(unittest.TestCase):
    ...
    def test_repository_seed_counts_are_stable(self):
        ...
        self.assertEqual(len(gs["catalogs"]["strings"]), 521)
        ...
        self.assertEqual(len(gs["catalogs"]["item_descriptions"]), 250)
        self.assertEqual(len(gs["catalogs"]["species_dex_text2"]), 251)
        self.assertIn("拍打", gs["catalogs"]["move_descriptions"]["POUND"])
        self.assertEqual(gs["catalogs"]["species_dex_text2"]["BULBASAUR"], "种子会跟着身体一起变大。")
        crystal_only = json.loads(
            (Path(__file__).parents[1] / "config" / "gsc" / "engine_scope_exclusions.json")
            .read_text(encoding="utf-8")
        )["excluded_keys"]
        self.assertEqual(set(crystal_only) - set(gs["catalogs"]["strings"]), set())
