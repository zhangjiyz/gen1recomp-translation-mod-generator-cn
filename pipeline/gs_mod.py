...
GS_OPTIONAL_VERIFIED_REGISTRIES = (
    "species_dex_text2", "move_descriptions", "item_descriptions", "status_labels",
)
...
def _write_gate_expectations(
    ...
    required_names = set(required_registries)
    if not catalog_names >= required_names:
        missing = sorted(required_names - catalog_names)
        raise BuildError(
            "Gold registry gate expectations are incomplete"
            + (f"; missing: {', '.join(missing)}" if missing else "")
        )
    extra = sorted(catalog_names - required_names - optional)
    if extra:
        raise BuildError(
            "Gold registry gate expectations are incomplete"
...
def build_gs_zh_seed_mod(
    ...
    *,
    ...
    for name, filename in id_sources.items():
        ...
        registry_stats[name] = {"translated": len(values), "total": len(current_ids)}
    ...
    if profile == UPSTREAM_PROFILE:
        source = Path(engine_source).resolve() / "src"
        if not source.is_dir():
            raise ValueError(f"upstream engine source has no src directory: {engine_source}")
        revision = checkout_revision(engine_source)
    else:
        source, _root, revision = verified_source(engine_source, manifest)
    all_engine_keys, gen2_engine_keys = engine_string_keys(iter_callsites(source), manifest)
    string_values = {
    ...
    coverage = {
        **pointer_coverage,
        ...
        "rom_catalogs": {
            "translated": registry_translated,
            "total": registry_total,
            "percent": round(100.0 * registry_translated / registry_total, 2)
            if registry_total else 100.0,
        },
        "rom": {
            "translated": rom_translated,
            "total": rom_total,
            "percent": round(100.0 * rom_translated / rom_total, 2)
            if rom_total else 100.0,
        },
        "engine": {
    ...
    stats = {
        ...
        "_gate_edition_dex_text": {},
        "_gate_crystal_catalogs": {},
